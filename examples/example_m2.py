...
parser = argparse.ArgumentParser(description="Process VTP file for merged airfoil meshing.")
...
# Select airfoil including TE: panel_id >= 0 or panel_id == min(panel_id)
min_panel_id = section_mesh.cell_data["panel_id"].min()
airfoil = pv.merge([
    section_mesh.threshold(value=(0, section_mesh.cell_data["panel_id"].max()), scalars="panel_id"),
    sort_points_by_y(section_mesh.threshold(value=(min_panel_id, min_panel_id), scalars="panel_id")),
])
...
def get_thickness_arrays(mesh):
    ...
    thickness_keys = [k for k in mesh_point.point_data.keys() if re.match(r"ply_.*_thickness", k)]
...
for i, (key, thickness_array) in enumerate(airfoil_thicknesses.items(), start=1):
    skins[f"skin{i}"] = Skin(thickness=Thickness(type="array", array=list(thickness_array)), material=material_id, sort_index=i)
    material_id += 1

# Define webs
web_definition = {}
web_meshes = [("web1", web1_thicknesses, web_points_2d_1), ("web2", web2_thicknesses, web_points_2d_2)]
for web_name, thicknesses, points in web_meshes:
    plies = [Ply(thickness=Thickness(type="array", array=list(thicknesses[key])), material=material_id + j) for j, key in enumerate(thicknesses)]
    material_id += len(thicknesses)
    normal_ref = [1, 0] if web_name == "web1" else [-1, 0]
    web_definition[web_name] = Web(coord_input=points, plies=plies, normal_ref=normal_ref)
